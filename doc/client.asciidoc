...
gl_fragment_program::
    ...
    implementation. This extension is used for warping effect on liquid
    surfaces when using legacy OpenGL backend. Default value is 1 (enabled).
    Ignored if ‘gl_shaders’ is enabled.

gl_shaders::
    Enables experimental GLSL rendering backend. This requires at least OpenGL
    3.1 and changes how ‘gl_modulate’, ‘gl_brightness’ and ‘intensity’
    parameters work. Default value is 0 (disabled).
