...
al_merge_looping::
    ...
    loudness compared to DMA sound engine. Default value is 1 (enabled). This
    method only works in stereo OpenAL speaker configuration (no HRTF). Users
    of systems with more than 2 speakers (or HRTF) should disable this. If set
    to 0, each source is played discretely, but they can be too loud or
    desynchronized.
