...
cl_async::
    ...
    the table below.
        ...
        retrace frequency (supported only when using SDL 1 with X11/GLX)
...
gl_vertex_buffer_object::
    ...
    static memory area, which speeds up rendering. Default value is 1
    (enabled).

gl_video_sync::
    Enables ‘GLX_SGI_video_sync’ extension when using SDL 1 with X11/GLX. This
    extension allows synchronizing rendering framerate to monitor vertical
    retrace frequency. Default value is 1 (enabled). See also ‘cl_async’
    variable.
