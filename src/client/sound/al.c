...
static void AL_PlayChannel(channel_t *ch)
{
    ...
    if (ch->dist_mult > 0) {
        qalSourcef(ch->srcnum, AL_MAX_DISTANCE, SOUND_FULLVOLUME + 1.0f / ch->dist_mult);
        qalSourcef(ch->srcnum, AL_ROLLOFF_FACTOR, 1.0f);
    } else {
        qalSourcef(ch->srcnum, AL_MAX_DISTANCE, 8192.0f);
