...
static bool parse_string(char *data, int depth)
{
    char *p, *cmd;
    int argc;
...
static bool parse_file(const char *path, int depth)
{
    ...
    bool result = parse_string(raw, depth);
...
// generated by meson
extern const char *const res_q2pro_menu;

void UI_LoadScript(void)
{
    if (!parse_file(UI_DEFAULT_FILE, 0)) {
        char *copy = Z_CopyString(res_q2pro_menu);
        parse_string(copy, 0);
        Z_Free(copy);
    }
}
